<div class="card card-navy">
    <div class="card-header">
        <h3 class="card-title">
            ...
            @else
                Registradas [ <b class="text-warning">{{ $rowsMarcas }}</b> ]
            @endif
        </h3>

        <div class="card-tools">
            <button type="button" class="btn btn-tool" wire:click="limpiarMarcas">
                <i class="fas fa-sync-alt"></i>
            </button>
            <button type="button" class="btn btn-tool" wire:click="create"
                    @if(!comprobarPermisos('marcas.create')) disabled @endif>
                <i class="fas fa-file"></i> Nuevo
            </button>
            <button type="button" class="btn btn-tool" wire:click="setLimit"
                    @if(($rows > $rowsMarcas) || ($keyword && $rows > $totalBusqueda)) disabled @endif >
        ...
        <table class="table table-sm table-head-fixed table-hover text-nowrap">
            ...
            @if($listarMarcas->isNotEmpty())
                @foreach($listarMarcas as $marca)
                    <tr>
                        <td class="text-uppercase text-truncate" style="max-width: 150px;">{{ $marca->nombre }}</td>
                        <td class="justify-content-end">
                            <div class="btn-group">
                                <button wire:click="edit('{{ $marca->rowquid }}')" class="btn btn-primary btn-sm"
                                        @if(!comprobarPermisos('marcas.edit')) disabled @endif >
                                    <i class="fas fa-edit"></i>
                                </button>

                                <button wire:click="destroy('{{ $marca->rowquid }}')" class="btn btn-primary btn-sm"
