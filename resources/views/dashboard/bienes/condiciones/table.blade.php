<div class="card card-navy">
    <div class="card-header">
        <h3 class="card-title">
            ...
            @else
                Registradas [ <b class="text-warning">{{ $rowsCondiciones }}</b> ]
            @endif
        </h3>

        <div class="card-tools">
            <button type="button" class="btn btn-tool" wire:click="limpiarCondiciones">
                <i class="fas fa-sync-alt"></i>
            </button>
            <button type="button" class="btn btn-tool" wire:click="create"
                    @if(!comprobarPermisos('condiciones.create')) disabled @endif>
                <i class="fas fa-file"></i> Nuevo
            </button>
            <button type="button" class="btn btn-tool" wire:click="setLimit"
                    @if(($rows > $rowsCondiciones) || ($keyword && $rows > $totalBusqueda)) disabled @endif >
        ...
        <table class="table table-sm table-head-fixed table-hover text-nowrap">
            ...
            @if($listarCondiciones->isNotEmpty())
                @foreach($listarCondiciones as $condicion)
                    <tr>
                        <td class="text-uppercase text-truncate" style="max-width: 150px;">{{ $condicion->nombre }}</td>
                        <td class="justify-content-end">
                            <div class="btn-group">
                                <button wire:click="edit('{{ $condicion->rowquid }}')" class="btn btn-primary btn-sm"
                                        @if(!comprobarPermisos('condiciones.edit')) disabled @endif >
                                    <i class="fas fa-edit"></i>
                                </button>

                                <button wire:click="destroy('{{ $condicion->rowquid }}')" class="btn btn-primary btn-sm"
